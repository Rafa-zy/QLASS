...
class Q_Trainer(Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            #dataloader_params["generator"] = torch.Generator(device='cuda')
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        """
        # if self.label_smoother is not None and "labels" in inputs:
            # labels = inputs.pop("labels").to(torch.bfloat16)
        # else:
        #     labels = None
        ...
        loss = torch.nn.MSELoss()(outputs.view(-1, outputs.size(-1)).squeeze(), labels.reshape(-1))
    ...
    def create_accelerator_and_postprocess(self):
        ...
        self.accelerator = Accelerator(**args, kwargs_handlers=kwargs)
